...
function writeQnaReply(qnano, qnareply, success, fail) {
  api.post(`/qna/${qnano}/reply`, qnareply).then(success).catch(fail);
}

function listQnaReply(qnano, success, fail) {
  api.get(`/qna/${qnano}/reply`).then(success).catch(fail);
}
export { listQna, writeQna, getQna, modifyQna, deleteQna, writeQnaReply, listQnaReply };
